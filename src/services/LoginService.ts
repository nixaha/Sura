...
export class LoginService {
    ...
    login(user: User): Promise<any> {
        return this.angfireAuth.auth.signInWithEmailAndPassword(user.email, user.password);
    }

    register(user: User): Promise<any> {
        return this.angfireAuth.auth.createUserWithEmailAndPassword(user.email, user.password);
    }
    ...
    setUserInfo(user) {
        this.angfireAuth.auth.onAuthStateChanged((usr) => {
            if (usr) {
                this.angfireDB.database.ref('Users').child(usr.uid).set(user);
            }
        });
    }
    ...
    getRegUsrErrorMessage(code, email): String {
        switch (code) {
            case strings.registroUsuarioErrors.emailRegistradoCode:
                return `La cuenta de correo <b>${email}</b> ${strings.registroUsuarioErrors.emailRegistradoMsg}`;
            default:
                return strings.registroUsuarioErrors.defaultRegistroUsr;
        }
